({ 
    ...
    { 
        ...
        component.find("newRecordCreator").getNewRecord( "Beer_Order__c", null, false, $A.getCallback(function() 
          { 
            ...
            if(error || (rec === null)) { console.log("Error initializing record template: " + error);
          }
          else 
              { 
    ...
    { 
        ...
        if(component.get("v.beerOrder.Same_as_Shipping_Address__c"))
        { 
        ...
        if(!isValid) return; var userId = $A.get("$SObjectType.CurrentUser.Id");
        //alert(userId); 
